...
# parameters (maybe in which 
# rides were randomly assigned vs one in which they
...
# === status freq ==== NOT SAFE TO DELETE
# finding the frequency of each status at each time step
status_freq <- table(visitor_data$time_step, visitor_data$status)

# generating data file and saving in data folder
status_freq_file <- paste(p.data, "/", "StatusFrequencies.csv", sep = "")

# adding visitor tracker (first time step)
write.csv(status_freq, status_freq_file)
...
#___ end of 4. analysis_________________________________________________________
